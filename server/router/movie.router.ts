...
export function makeMovieRouter(movieService: IMovieService, userService: IUserService): Express.Express {
    ...
    // POST or PUT /movie 
    movieRouter.put("/", checkAuth, async (req: Express.Request, res: Express.Response) => {
        try {
            ...
            res.status(201).send({ respons: "success", data: movie });
        } catch (e: any) {
            res.status(500).send(e.message);
        }
    });
    ...
    // DELETE /movie/id 
    movieRouter.delete("/:id", checkAuth, async (req: Express.Request, res: Express.Response) => {
        try {
            const ms = await movieService;
            const id: number = parseInt(req.params.id, 10);
            ...
            res.status(201).send({ respons: "success", data: "Movie is deleted\n" });
        } catch (e: any) {
            res.status(500).send(e.message);
        }
    });
    ...
    movieRouter.delete("favorite/:id", checkAuth, async (req: Express.Request, res: Express.Response) => {
        try {
            const id: number = parseInt(req.params.id, 10);
...
export function makeDefaultMovieRouter(): Express.Express {
    return makeMovieRouter(movieDBService, userDBService);
}
